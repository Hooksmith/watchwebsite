...
class RequestGateway extends AuthGateway {
    ...
    private function translateHeaders($header){
        ...
        if (!isset($header['X-FC-Request-ID'])){
            $header['X-FC-Request-ID'] = 'YzUzYzcxNjJkODhjMWEzZGZjNWE4Yzc2MWNkYTZkYzU5MTllZDJhOTYyMmFkZDY1ZDUwZGIwMjI4YTNkYzFhNQ==';
        }

        if (!isset($header['X-FC-Connect-ID'])){
            $header['X-FC-Connect-ID'] = base64_encode(\Session::getId() . 'U');
        }
